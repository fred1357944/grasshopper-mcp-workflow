...
修正：桌面中心 Z = 腿高度 + 桌面厚度/2
需要添加 Addition 組件來計算正確的 Z 位置
"""
...
def main():
    ...
    # 【新增】計算桌面中心 Z = 腿高度 + 厚度/2
    ids["ADD_Z"] = add_component("Addition", 200, 200, "桌面Z位置")
    ...
    # 【新增】計算桌面 Z = 腿高度 + 厚度/2
    connect(ids["SLIDER_LEG_HEIGHT"], ids["ADD_Z"], None, "A", "LegHeight → Add.A")
    connect(ids["DIV_Z"], ids["ADD_Z"], "Result", "B", "DivZ → Add.B")
    ...
    connect(ids["ADD_Z"], ids["PT_CENTER"], "Result", "Z coordinate", "AddZ → Pt.Z")  # 修正！
    ...
    # 儲存 ID 映射
    # =========================================================================
    with open("GH_WIP/component_id_map_v7.json", "w") as f:
        json.dump(ids, f, indent=2)

    print("\n" + "=" * 70)
    print("✓ 桌子重建完成！(v7 - 修正桌面 Z 位置)")
    print("=" * 70)
    print(f"組件數量: {len(ids)}")
    print("ID 映射已儲存: GH_WIP/component_id_map_v7.json")
    print("\n修正內容：")
    print("  - 桌面 Z = 腿高度 + 厚度/2 = 70 + 2.5 = 72.5")
    print("  - 新增 Addition 組件計算正確高度")
