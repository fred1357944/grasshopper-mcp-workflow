...
def _rule_based_intent_parsing(state: GraphState, design_intent: str) -> GraphState:
    ...
    # 識別意圖類型
    if any(kw in intent_lower for kw in ["螺旋", "spiral", "helix"]):
        state["intent_type"] = "curve_generation"
        state["core_operations"] = ["spiral_movement", "parametric_curve"]
        state["matched_intent_patterns"] = ["trigonometric_spiral"]
    ...
    elif any(kw in intent_lower for kw in ["陣列", "array", "複製", "copy"]):
        state["intent_type"] = "distribution"
        state["core_operations"] = ["array_distribution"]
    else:
    ...
    # 識別參數化需求
    params = []
    if any(kw in intent_lower for kw in ["圈數", "turns", "rounds"]):
        params.append({"name": "turns", "type": "number", "suggested_range": [1, 10]})
    if any(kw in intent_lower for kw in ["半徑", "radius"]):
        params.append({"name": "radius", "type": "number", "suggested_range": [1, 50]})
    if any(kw in intent_lower for kw in ["高度", "height"]):
        params.append({"name": "height", "type": "number", "suggested_range": [1, 100]})
    if any(kw in intent_lower for kw in ["數量", "count", "個"]):
        params.append({"name": "count", "type": "number", "suggested_range": [1, 100]})
    
    state["parametric_requirements"] = params
    state["intent_confidence"] = 0.6 if state["intent_type"] != "unknown" else 0.3
        ...
        gh_code = {
            "components": [
                ...
                {"id": "slider_height", "type": "Number Slider", "nickname": "Height",
                 "position": [50, 200], "properties": {"min": 1, "max": 100, "value": 30}},
            ...
            "connections": [
                {"from": {"component": "slider_turns", "output": 0}, 
                 "to": {"component": "series", "input": "Count"}},
            ...
            "sliders": [
                ...
                {"id": "slider_height", "name": "高度", "min": 1, "max": 100, "default": 30},
            ]
        }
    else:
        # 通用空模板
