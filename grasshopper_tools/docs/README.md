...
```python
from grasshopper_tools import MMDParser, JSONGenerator
...
group_mgr = GroupManager(client, comp_mgr)
    ...
    # ]
    # success, fail = comp_mgr.add_components_parallel(add_commands, max_workers=10)
    # comp_mgr.save_id_map()  # 保存 ID 映射
    #
    # connect_commands = [
    #     {
    ...
    # ]
    # success, fail = conn_mgr.connect_components_parallel(connect_commands, max_workers=10)
...
### 新代碼
```python
from grasshopper_tools import GrasshopperClient
...
### 新代碼
```python
from grasshopper_tools import ParameterSetter, ComponentManager
