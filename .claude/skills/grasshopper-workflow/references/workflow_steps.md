...
7. 請先確認所有slider的數值是否設置正確是否有對齊component_info.mmd規劃的數值，并使用"set_slider_properties"更新
   - 推薦使用自動化腳本: python GH_WIP/auto_set_sliders.py
   - 腳本會自動從component_info.mmd提取所有Number Slider的輸出值並設置
   - 腳本會自動從component_id_map.json讀取實際組件ID（由execute_placement.py自動生成）
   - 腳本會根據slider名稱自動確定合理的範圍（min/max）
8.使用"group_components"群組電池可以參考component_info.mmd
   - 推薦使用自動化腳本: python GH_WIP/auto_group_components.py
   - 腳本會自動從component_info.mmd提取所有subgraph定義及其組件列表
   - 腳本會根據subgraph類型自動分配顏色（TOP=淺藍色, LEG_BASE=淺橙色, LEG_PLANES=淺綠色, ORIENT_GROUP=淺紅色）
   - 腳本會自動從component_id_map.json讀取實際組件ID（由execute_placement.py自動生成）

執行批次創建命令的方法:

使用execute_placement.py腳本
- 執行命令: python GH_WIP/execute_placement.py
- 腳本會自動：
...
1. auto_set_sliders.py - 自動設置Slider參數
   - 功能：從component_info.mmd自動提取所有Number Slider的輸出值並設置
   - 執行：python GH_WIP/auto_set_sliders.py
...
2. auto_group_components.py - 自動群組組件
   - 功能：從component_info.mmd自動提取subgraph定義並進行群組
   - 執行：python GH_WIP/auto_group_components.py
   - 前置條件：需先執行execute_placement.py生成component_id_map.json
   - 工作流程：
...
推薦執行順序:
1. 執行execute_placement.py創建組件和連接（自動生成component_id_map.json）
2. 使用get_document_errors檢查並修正錯誤
3. 執行auto_set_sliders.py自動設置所有slider參數
4. 執行auto_group_components.py自動群組所有組件
